...
def modify_format(modelName):
    ...
    def write_layer(file_name, model_name, layer_type, layer_number, input_param, output_param, weight_param,
                   hidden_param):
        default_input_param = dict().fromkeys(['input_batch', 'input_x', 'input_y', 'input_channel'], '1')
        default_output_param = dict().fromkeys(['output_batch', 'output_x', 'output_y', 'output_channel'], '1')
        default_weight_param = dict().fromkeys(
        ...
        with open("result.m", 'a') as f2:
            #f2.write('Model_Name = ' + re.sub('\.py','',model_name) + '\n')
            f2.write('Layer ' + layer_type + layer_number + ' {\n')
            f2.write('Type: ' + layer_type + '\n')
            ip = replace_param(default_input_param,input_param)
            #ip['input_batch']='1'
            op = replace_param(default_output_param,output_param)
            #op['output_batch']='1'
            ...
            print(op)
            print(wp)
            f2.write('Dimensions { K ' + op['output_channel']
                    + ',C ' + ip['input_channel']
                    + ',R ' + wp['weight_y']
                    + ',S ' + wp['weight_x']
                    + ',Y ' + op['output_y']
    ...
    def modify_conv():
        with open(convPath, 'r') as f:
            for line in f.readlines():
                ...
                write_layer(filename, modelName, 'CONV', number.strip(), Input_param, Output_param, Weight_param,
                           dict())
